		...
		<div class="relative mx-auto flex min-h-screen max-w-6xl items-center justify-center px-6 py-12">
			<div class="w-full max-w-md rounded-2xl border border-white/10 bg-white/5 p-8 shadow-2xl shadow-black/40 backdrop-blur">
				<div class="mb-6 space-y-2 text-center">
					...
					<p class="text-sm text-slate-300">Provision secure access for your institution.</p>
				</div>

				<form class="space-y-4" @submit.prevent="submit">
					<div class="space-y-2">
						<label class="text-xs font-semibold text-slate-200" for="name">Full name</label>
						<input
							id="name"
							v-model="form.name"
							type="text"
					...
					<div class="space-y-2">
						<label class="text-xs font-semibold text-slate-200" for="email">Institutional email</label>
						<input
							id="email"
							v-model="form.email"
							type="email"
					...
					<div class="space-y-2">
						<label class="text-xs font-semibold text-slate-200" for="password_confirmation">Confirm password</label>
						<input
							...
							autocomplete="new-password"
						/>
						<p v-if="form.errors.password_confirmation" class="text-xs text-red-400">{{ form.errors.password_confirmation }}</p>
					</div>

					<div class="flex items-start gap-3 text-xs text-slate-300">
						<input v-model="form.agree" id="agree" type="checkbox" class="mt-1 h-4 w-4 rounded border-white/20 bg-white/5 text-[var(--accent)] focus:ring-[var(--accent)]" />
						<label for="agree" class="leading-snug">
							I agree to the terms and privacy policy for authorized institutional use.
						</label>
					</div>
					<p v-if="form.errors.agree" class="text-xs text-red-400">{{ form.errors.agree }}</p>
				...
				<div class="mt-6 text-center text-xs text-slate-400">
					...
					<Link class="ml-2 font-semibold text-[var(--accent)] transition hover:text-white" href="/login">Back to login</Link>
				</div>
			</div>
		</div>
	</div>
